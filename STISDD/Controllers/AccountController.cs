...
namespace STISDD.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    //if (string.Equals(userRole.Role.Name, "Employee"))
                    //{
                        //Session["EmployeeId"] = db.Doctors.Where(d => d.LoginId == loginId).First().DoctorId;
                    //}
                    //else if (string.Equals(userRole.Role.Name, "Guest"))
                    //{
                        //Session["GuestId"] = db.Collegians.Where(c => c.LoginId == loginId).First().CollegianId;
                    //}
                    return RedirectToLocal(returnUrl);
                }
                else
                {
                    ModelState.AddModelError("", "نام کاربری یا رمز عبور معتبر نیست.");
                }
            }
        ...
        {
            ViewBag.RoleId = new SelectList(new[] { new { Id = 0, Name = "کارمند" }, new { Id = 1, Name = "مهمان" } }, "Id", "Name", 1);
            return View();
        }
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.ChangePasswordSuccess ? "گذر واژه شما تغییر کرده است."
                : message == ManageMessageId.SetPasswordSuccess ? "گذر واژه شما تنظیم شده است."
                : message == ManageMessageId.RemoveLoginSuccess ? "ورود به سیستم خارجی حذف شد."
