...
namespace EMR.Areas.Clinic.Controllers
{
    public class CardiologyController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Create([Bind()] CardiologyVisit cardiologyVisit, FormCollection fCVisit)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (historyDiseas == null)
                    {
                        cardiologyVisit.HistoryDiseasVmp.PatientId = Convert.ToInt32(fCVisit["PatientId"]);
                    ...
                    if (fCVisit["FollowUpDate"] == string.Empty)
                        cardiologyVisit.FollowUpVmp.Date = Convert.ToDateTime(fCVisit["DateFllowUp"].ToString());
